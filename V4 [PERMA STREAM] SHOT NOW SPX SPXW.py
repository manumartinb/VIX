# -*- coding: utf-8 -*-
r"""
SHOT NOW V3 — Foto estática AHORA (SPXW+SPX) + Postprocesado (IV_BS, r y griegas BS) + Modo PERMA cada 10'
----------------------------------------------------------------------------------------------------------
...
- Añade ÚNICAMENTE el mecanismo de ejecución recurrente de PERMA STREAM:
    ...
    * Cadencia: cada 10 minutos, alineado a múltiplos (…:00, …:10, …:20, …:30, …:40, …:50).
...
PRETTY_TIME = True
HEADERS = {
    "User-Agent": "TD-Snapshot-ONCE/3.0 (+httpx)",
...
# ================== Bucle PERMA RTH cada 10' (clonado de PERMA STREAM, sin compresión) ==================
def run_perma_rth_every_10min():
    print(f"[{timestamp_now()}] === SHOT NOW V3 — Modo PERMA {CADENCE_MIN}min (RTH 09:30–16:00 ET; sin fines de semana) ===")
    while True:
        now = et_now()
...
def main():
    ...
    # Ejecuta SHOT NOW en modo PERMA (cada 10' dentro de RTH, saltando fines de semana)
    run_perma_rth_every_10min()
